...
describe("GET /companies", function () {
	...
	test("works with filters", async function () {
		const resp = await request(app).get(
			"/companies?name=c2&minEmployees=2&maxEmployees=2"
		);
		expect(resp.body).toEqual({
			companies: [
				{
	...
	test("doesnt break with invalid filters added to query", async function () {
		const resp = await request(app).get("/companies?name=c2&notValid=fake");
		expect(resp.body).toEqual({
			companies: [
				{
					handle: "c2",
	...
	test("throws error when min is greater than max", async function () {
		const resp = await request(app).get("/companies?minEmployees=4&maxEmployees=2");
    expect(resp.body).toEqual({
        error: {
          message: "min employees can not be greater the max employees",
          status: 400,
        },
      },
    );
